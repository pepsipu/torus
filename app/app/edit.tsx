...
export default function EditFollowingScreen() {
  ...
  useEffect(() => {
    const myFollowingSection = searchEntities.sections.find(
      (section) => section.id === "my_following"
    );
    const items = myFollowingSection
      ? getAllEntitiesForSection("my_following")
      : [];
  ...
  const HeaderComponent = ({
    ...
    <Header
      ...
      headerCenter={<Text className="text-2xl font-bold">Edit Following</Text>}
      headerLeft={
  ...
  const LargeHeaderComponent = () => {
    ...
    return (
      <View
        className="px-4 pt-6 pb-3 bg-white"
    ...
    return (
      <ScaleDecorator activeScale={0.95}>
        <TouchableOpacity
          onLongPress={drag}
          disabled={isActive}
          className={`flex-row items-center bg-white mx-2 px-4 py-2 rounded-xl ${
