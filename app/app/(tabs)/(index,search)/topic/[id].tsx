...
export default function TopicScreen() {
  ...
  const filteredScores = entity?.filter
    ? scores.filter((score) => score.sports_type === entity.filter)
    : [];

  // Replace the dummyNews with filtered news
  const filteredNews = React.useMemo(() => {
    return (news as NewsItemType[]).filter(
      (item) =>
        item.topic?.id === id || item.related_topics?.includes(id as string)
    );
  }, [id]);
        ...
        <View className=" border-b border-gray-200">
          <View className="flex-row justify-between items-center p-4 gap-4">
            ...
            <TouchableOpacity
              ...
              className="p-2"
            >
              <Text className="font-semibold color-apple-news text-lg">
                Done
              </Text>
            </TouchableOpacity>
          </View>

          <Text className="text-xl font-bold px-4 pb-2">
            {entity.sub_topics?.title}
          </Text>
          ...
          {entity.sub_topics?.items.map((item, index) => (
            ...
            >
              ...
              <Text className="text-xl font-semibold tracking-tight flex-1">
                {item.title}
              </Text>
                ...
                >
                  {entity.sub_topics.items.map((item, index) => (
                    ...
                    >
                      ...
                      />
                      <Text
                        className="text-center font-semibold text-md tracking-tighter"
                        numberOfLines={1}
                      >
                        {item.title}
                      </Text>
  ...
  return (
    <View
      className={`flex-1 ${
        Platform.OS !== "web" ? "bg-[#F2F2F7]" : "bg-white"
      }`}
    >
      <ScrollViewWithHeaders
