...
const SearchComponent = React.memo(
  ({ value, onChangeText }: SearchComponentProps) => {
...
export default function SearchScreen() {
  ...
  const searchResults = useMemo(() => {
    ...
    Object.values(entities).forEach((entity: any) => {
      ...
      if (
        (matchesTitle || matchesDescription || matchesType) &&
  ...
  const HeaderComponent = ({
    ...
    <Header
      ...
      headerCenter={<Text className="text-2xl font-bold">Following</Text>}
      headerRight={
  ...
  const LargeHeaderComponent = () => {
    ...
    return (
      <View
        className={`px-4 pt-2 pb-3  gap-3 ${
          Platform.OS !== "web" ? "#F2F2F7" : "white"
        }`}
    ...
    <>
      {Platform.OS === "web" && (
        <Head>
          ...
          <meta
            name="keywords"
            content="apple news search, news search, article search, topic search"
          />
        </Head>
      )}
          ...
          <>
            ...
            {searchEntities.sections.map((section) => (
              <AnimatedAccordion
                key={section.id}
                title={section.title}
                openDefault={section.id === "my_following"}
              >
