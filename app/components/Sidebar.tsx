...
export function Sidebar() {
  ...
  const liveGames = React.useMemo(
    () => scores.filter((game: Game) => game.is_live),
    []
  );
  ...
  return (
    <View
      className="w-[350px] sticky top-0 "
      style={{ borderLeftColor: "#eee" }}
    >
      <View
        className="w-[350px] border-l fixed top-0 px-2"
        style={{ borderLeftColor: "#eee" }}
      >
            ...
            {searchQuery ? (
              ...
              <View style={styles.defaultContent}>
                {searchEntities.sections.map((section) => {
                  if (
                    section.id === "my_following" ||
                    section.id === "special_coverage"
                  )
                    return null;
                  return (
