...
export const PodcastItem = memo(
  ({ episode, index, totalItems = 0 }: PodcastItemProps) => {
    ...
    const durationInMinutes = episode.attributes?.durationInMilliseconds
      ? Math.floor(episode.attributes.durationInMilliseconds / 60000)
      : null;
    ...
    const handleSeeDetails = () => {
      Alert.alert(
        "Details",
        episode.attributes?.description?.standard || "No details available"
      );
    };
    ...
    const remainingTime =
      isCurrentlyPlaying && episode.attributes?.durationInMilliseconds
        ? Math.floor(
            (episode.attributes.durationInMilliseconds - position.value) / 60000
          )
        : null;
