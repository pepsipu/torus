...
const MenuItems = ({ item }: { item: NewsItemType }) => {
  return (
    <>
      ...
      <ContextMenu.Separator />
      <ContextMenu.Item
        textValue={`Go to ${item.source.name}`}
        onSelect={() => {}}
        key="source"
      >
            ...
            <View className="mt-4">
              ...
              {item.related_news.map((news) => (
                <Pressable key={news.id} className="mb-4">
                  ...
                  <Text className="text-lg font-semibold mb-1 -tracking-[0.5px]">
                    {news.title}
                  </Text>
                </Pressable>
              ))}
        ...
        <ContextMenu.Content>
          ...
          <ContextMenu.Sub>
            <ContextMenu.SubTrigger
              key="source"
              textValue={` ${item.source.name}`}
            >
              <ContextMenu.ItemIcon>
                <MaterialIcons name="open-in-new" size={18} />
              </ContextMenu.ItemIcon>
              <ContextMenu.ItemTitle>
                Go to {item.source.name}
              </ContextMenu.ItemTitle>
            </ContextMenu.SubTrigger>
            <ContextMenu.SubContent>
              ...
              <ContextMenu.Item
                key="notifications"
                onSelect={() => {}}
                textValue="Turn on Notifications"
              >
