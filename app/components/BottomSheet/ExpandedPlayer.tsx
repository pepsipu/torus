...
export function ExpandedPlayer({ scrollComponent }: ExpandedPlayerProps) {
  ...
  const formattedPosition = useDerivedValue(() =>
    formatTime(currentPosition.value)
  );
  const formattedRemaining = useDerivedValue(
    () =>
      `-${formatTime(
        Math.max(0, currentDuration.value - currentPosition.value)
      )}`
  );
  ...
  const renderContent = () => {
    ...
    return (
      <View
        ...
        className={Platform.OS === "ios" ? "mx-5 gap-8" : ""}
      >
        {Platform.OS === "web" && (
          <Pressable
            onPress={() => {
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.progressContainer}>
            <View style={styles.progressBar}>
              <View
                style={[styles.progress, { width: `${progress.value}%` }]}
              />
            </View>
